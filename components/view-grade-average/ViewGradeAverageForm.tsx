...
const GradeAverageForm: React.FC = () => {
  ...
  const handleDownloadTemplate = () => {
    // Create CSV template
    const headers = ["ID", "Trainee Name", "Email", "Assignment Average", "Final Average"];
    const csvContent = [
      headers.join(","),
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          ...
          const newStudents = lines.map((line, index) => {
            const [id, name, email, assignmentAverage, finalAverage] = line.split(',');
            return {
              id: parseInt(id),
              name,
              email,
  ...
  const handleExport = () => {
    ...
    const headers = ["ID", "Trainee Name", "Email", "Assignment Average", "Final Average", "Total"];
    const csvContent = [
      headers.join(","),
      ...students.map(student => 
        [
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 ml-64 bg-[#EFF5EB]">
        <div className="py-8 px-12 mt-10">
          <div className="mb-8">
            <h1 className="text-4xl font-bold mb-8">Grade Average: HN24_CPL_JAVA_01</h1>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[#6FBC44] text-white">
                  <th className="py-4 px-6 text-left w-16 border-r border-[#5da639]">#</th>
                  <th className="py-4 px-6 text-left border-r border-[#5da639]">Trainee Name</th>
                  <th className="py-4 px-6 text-left border-r border-[#5da639]">Email</th>
              ...
              <tbody>
                {students.map((student) => (
                  <tr key={student.id} className="border-b hover:bg-gray-50">
                    <td className="py-4 px-6 border-r border-gray-200">{student.id}</td>
                    <td className="py-4 px-6 border-r border-gray-200">{student.name}</td>
                    <td className="py-4 px-6 border-r border-gray-200">{student.email}</td>
